...
### Positive
- **Transport Reusability**: CloudVitta serves pricing through both REST endpoints and MCP agent tools. A framework's middleware (e.g., Gin's rate limiter) only works for REST. By centralizing cross-cutting concerns (auth, rate limiting) into reusable internal packages that wrap standard Go interfaces, we write the logic once and apply it cleanly across both REST and MCP, rather than coupling it to a REST-only framework.
...
Rejected. Framework-specific middlewares (such as Gin rate limiters or auth handlers) cannot be reused for MCP tool servers or background jobs. They introduce third-party API churn and dependency bloat without offering benefits over the standard library `http.ServeMux` (Go 1.22+).
