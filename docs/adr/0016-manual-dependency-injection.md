...
As CloudVitta's complexity grows to include 7 cloud provider adapters, transport interfaces (REST and MCP), caches, and FX services, wiring the application dependency graph in `main.go` could become verbose.
...
- **Avoids Codegen Overhead**: While `wire` preserves compile-time safety, the actual dependency graph for CloudVitta is shallow. Seven providers sharing one interface is one wiring pattern repeated; transports taking one service layer is trivial. The graph is not deep or combinatorial enough to justify the overhead of a codegen step.
