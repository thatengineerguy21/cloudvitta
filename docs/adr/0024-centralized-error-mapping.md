...
CloudVitta handles requests across multiple transport boundaries (REST, gRPC, MCP, A2A). When the core business logic (`internal/service`) encounters an error (e.g., a validation failure or missing data), that error must be translated into a protocol-specific response (e.g., an HTTP 400 with an RFC 7807 JSON payload).
...
- **Clean Handlers**: HTTP/gRPC handlers remain incredibly thin, completely decoupled from error serialization logic.
