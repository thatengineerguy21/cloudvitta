...
// KnownGCPTransferTypes returns a copy of known GCP transfer type mappings.
func KnownGCPTransferTypes() map[string]string {
	m := make(map[string]string, len(gcpTransferTypeMap))
	for k, v := range gcpTransferTypeMap {
		m[k] = v
	}
	return m
}
